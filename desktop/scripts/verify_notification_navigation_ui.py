...
def verify_navigation_ui(page, context, base, native, output_dir):
    ...
    def messages(route):
        ...
        route.fulfill(json={'success': True, 'data': {'hasMore': False, 'messages': [dict(
            messageId=account + cid, senderId='fixture', senderName='验收买家', isSelf=False,
    ...
    pending.append(dict(id='click-b', account_id='shop-b', chat_id='outside-page', buyer_id='buyer-b'))
    ...
    expect(page.get_by_role('heading', name='闲鱼用户 buyer-b', exact=True)).to_be_visible()
    ...
    print('Passed: native generic activation; notification cross-shop/exact-cid/outside-first-page/refresh/manual-selection; order nickname + ID + search (offline UI fixtures)', flush=True)
